/*
Jellyfin API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 10.8.13
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api

import (
	"encoding/json"
)

// checks if the AdminNotificationDto type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AdminNotificationDto{}

// AdminNotificationDto The admin notification dto.
type AdminNotificationDto struct {
	// Gets or sets the notification name.
	Name NullableString `json:"Name,omitempty"`
	// Gets or sets the notification description.
	Description NullableString `json:"Description,omitempty"`
	// Gets or sets the notification level.
	NotificationLevel NullableNotificationLevel `json:"NotificationLevel,omitempty"`
	// Gets or sets the notification url.
	Url NullableString `json:"Url,omitempty"`
}

// NewAdminNotificationDto instantiates a new AdminNotificationDto object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAdminNotificationDto() *AdminNotificationDto {
	this := AdminNotificationDto{}
	return &this
}

// NewAdminNotificationDtoWithDefaults instantiates a new AdminNotificationDto object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAdminNotificationDtoWithDefaults() *AdminNotificationDto {
	this := AdminNotificationDto{}
	return &this
}

// GetName returns the Name field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AdminNotificationDto) GetName() string {
	if o == nil || IsNil(o.Name.Get()) {
		var ret string
		return ret
	}
	return *o.Name.Get()
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AdminNotificationDto) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Name.Get(), o.Name.IsSet()
}

// HasName returns a boolean if a field has been set.
func (o *AdminNotificationDto) HasName() bool {
	if o != nil && o.Name.IsSet() {
		return true
	}

	return false
}

// SetName gets a reference to the given NullableString and assigns it to the Name field.
func (o *AdminNotificationDto) SetName(v string) {
	o.Name.Set(&v)
}
// SetNameNil sets the value for Name to be an explicit nil
func (o *AdminNotificationDto) SetNameNil() {
	o.Name.Set(nil)
}

// UnsetName ensures that no value is present for Name, not even an explicit nil
func (o *AdminNotificationDto) UnsetName() {
	o.Name.Unset()
}

// GetDescription returns the Description field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AdminNotificationDto) GetDescription() string {
	if o == nil || IsNil(o.Description.Get()) {
		var ret string
		return ret
	}
	return *o.Description.Get()
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AdminNotificationDto) GetDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Description.Get(), o.Description.IsSet()
}

// HasDescription returns a boolean if a field has been set.
func (o *AdminNotificationDto) HasDescription() bool {
	if o != nil && o.Description.IsSet() {
		return true
	}

	return false
}

// SetDescription gets a reference to the given NullableString and assigns it to the Description field.
func (o *AdminNotificationDto) SetDescription(v string) {
	o.Description.Set(&v)
}
// SetDescriptionNil sets the value for Description to be an explicit nil
func (o *AdminNotificationDto) SetDescriptionNil() {
	o.Description.Set(nil)
}

// UnsetDescription ensures that no value is present for Description, not even an explicit nil
func (o *AdminNotificationDto) UnsetDescription() {
	o.Description.Unset()
}

// GetNotificationLevel returns the NotificationLevel field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AdminNotificationDto) GetNotificationLevel() NotificationLevel {
	if o == nil || IsNil(o.NotificationLevel.Get()) {
		var ret NotificationLevel
		return ret
	}
	return *o.NotificationLevel.Get()
}

// GetNotificationLevelOk returns a tuple with the NotificationLevel field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AdminNotificationDto) GetNotificationLevelOk() (*NotificationLevel, bool) {
	if o == nil {
		return nil, false
	}
	return o.NotificationLevel.Get(), o.NotificationLevel.IsSet()
}

// HasNotificationLevel returns a boolean if a field has been set.
func (o *AdminNotificationDto) HasNotificationLevel() bool {
	if o != nil && o.NotificationLevel.IsSet() {
		return true
	}

	return false
}

// SetNotificationLevel gets a reference to the given NullableNotificationLevel and assigns it to the NotificationLevel field.
func (o *AdminNotificationDto) SetNotificationLevel(v NotificationLevel) {
	o.NotificationLevel.Set(&v)
}
// SetNotificationLevelNil sets the value for NotificationLevel to be an explicit nil
func (o *AdminNotificationDto) SetNotificationLevelNil() {
	o.NotificationLevel.Set(nil)
}

// UnsetNotificationLevel ensures that no value is present for NotificationLevel, not even an explicit nil
func (o *AdminNotificationDto) UnsetNotificationLevel() {
	o.NotificationLevel.Unset()
}

// GetUrl returns the Url field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AdminNotificationDto) GetUrl() string {
	if o == nil || IsNil(o.Url.Get()) {
		var ret string
		return ret
	}
	return *o.Url.Get()
}

// GetUrlOk returns a tuple with the Url field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AdminNotificationDto) GetUrlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Url.Get(), o.Url.IsSet()
}

// HasUrl returns a boolean if a field has been set.
func (o *AdminNotificationDto) HasUrl() bool {
	if o != nil && o.Url.IsSet() {
		return true
	}

	return false
}

// SetUrl gets a reference to the given NullableString and assigns it to the Url field.
func (o *AdminNotificationDto) SetUrl(v string) {
	o.Url.Set(&v)
}
// SetUrlNil sets the value for Url to be an explicit nil
func (o *AdminNotificationDto) SetUrlNil() {
	o.Url.Set(nil)
}

// UnsetUrl ensures that no value is present for Url, not even an explicit nil
func (o *AdminNotificationDto) UnsetUrl() {
	o.Url.Unset()
}

func (o AdminNotificationDto) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AdminNotificationDto) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Name.IsSet() {
		toSerialize["Name"] = o.Name.Get()
	}
	if o.Description.IsSet() {
		toSerialize["Description"] = o.Description.Get()
	}
	if o.NotificationLevel.IsSet() {
		toSerialize["NotificationLevel"] = o.NotificationLevel.Get()
	}
	if o.Url.IsSet() {
		toSerialize["Url"] = o.Url.Get()
	}
	return toSerialize, nil
}

type NullableAdminNotificationDto struct {
	value *AdminNotificationDto
	isSet bool
}

func (v NullableAdminNotificationDto) Get() *AdminNotificationDto {
	return v.value
}

func (v *NullableAdminNotificationDto) Set(val *AdminNotificationDto) {
	v.value = val
	v.isSet = true
}

func (v NullableAdminNotificationDto) IsSet() bool {
	return v.isSet
}

func (v *NullableAdminNotificationDto) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAdminNotificationDto(val *AdminNotificationDto) *NullableAdminNotificationDto {
	return &NullableAdminNotificationDto{value: val, isSet: true}
}

func (v NullableAdminNotificationDto) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAdminNotificationDto) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


