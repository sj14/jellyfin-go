/*
Jellyfin API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 10.11.10
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api

import (
	"encoding/json"
)

// checks if the SyncPlayGroupUpdateMessage type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SyncPlayGroupUpdateMessage{}

// SyncPlayGroupUpdateMessage Untyped sync play command.
type SyncPlayGroupUpdateMessage struct {
	// Group update data
	Data *GroupUpdate `json:"Data,omitempty"`
	// Gets or sets the message id.
	MessageId *string `json:"MessageId,omitempty"`
	// The different kinds of messages that are used in the WebSocket api.
	MessageType *SessionMessageType `json:"MessageType,omitempty"`
}

// NewSyncPlayGroupUpdateMessage instantiates a new SyncPlayGroupUpdateMessage object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSyncPlayGroupUpdateMessage() *SyncPlayGroupUpdateMessage {
	this := SyncPlayGroupUpdateMessage{}
	return &this
}

// NewSyncPlayGroupUpdateMessageWithDefaults instantiates a new SyncPlayGroupUpdateMessage object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSyncPlayGroupUpdateMessageWithDefaults() *SyncPlayGroupUpdateMessage {
	this := SyncPlayGroupUpdateMessage{}
	return &this
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *SyncPlayGroupUpdateMessage) GetData() GroupUpdate {
	if o == nil || IsNil(o.Data) {
		var ret GroupUpdate
		return ret
	}
	return *o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SyncPlayGroupUpdateMessage) GetDataOk() (*GroupUpdate, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *SyncPlayGroupUpdateMessage) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given GroupUpdate and assigns it to the Data field.
func (o *SyncPlayGroupUpdateMessage) SetData(v GroupUpdate) {
	o.Data = &v
}

// GetMessageId returns the MessageId field value if set, zero value otherwise.
func (o *SyncPlayGroupUpdateMessage) GetMessageId() string {
	if o == nil || IsNil(o.MessageId) {
		var ret string
		return ret
	}
	return *o.MessageId
}

// GetMessageIdOk returns a tuple with the MessageId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SyncPlayGroupUpdateMessage) GetMessageIdOk() (*string, bool) {
	if o == nil || IsNil(o.MessageId) {
		return nil, false
	}
	return o.MessageId, true
}

// HasMessageId returns a boolean if a field has been set.
func (o *SyncPlayGroupUpdateMessage) HasMessageId() bool {
	if o != nil && !IsNil(o.MessageId) {
		return true
	}

	return false
}

// SetMessageId gets a reference to the given string and assigns it to the MessageId field.
func (o *SyncPlayGroupUpdateMessage) SetMessageId(v string) {
	o.MessageId = &v
}

// GetMessageType returns the MessageType field value if set, zero value otherwise.
func (o *SyncPlayGroupUpdateMessage) GetMessageType() SessionMessageType {
	if o == nil || IsNil(o.MessageType) {
		var ret SessionMessageType
		return ret
	}
	return *o.MessageType
}

// GetMessageTypeOk returns a tuple with the MessageType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SyncPlayGroupUpdateMessage) GetMessageTypeOk() (*SessionMessageType, bool) {
	if o == nil || IsNil(o.MessageType) {
		return nil, false
	}
	return o.MessageType, true
}

// HasMessageType returns a boolean if a field has been set.
func (o *SyncPlayGroupUpdateMessage) HasMessageType() bool {
	if o != nil && !IsNil(o.MessageType) {
		return true
	}

	return false
}

// SetMessageType gets a reference to the given SessionMessageType and assigns it to the MessageType field.
func (o *SyncPlayGroupUpdateMessage) SetMessageType(v SessionMessageType) {
	o.MessageType = &v
}

func (o SyncPlayGroupUpdateMessage) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SyncPlayGroupUpdateMessage) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Data) {
		toSerialize["Data"] = o.Data
	}
	if !IsNil(o.MessageId) {
		toSerialize["MessageId"] = o.MessageId
	}
	if !IsNil(o.MessageType) {
		toSerialize["MessageType"] = o.MessageType
	}
	return toSerialize, nil
}

type NullableSyncPlayGroupUpdateMessage struct {
	value *SyncPlayGroupUpdateMessage
	isSet bool
}

func (v NullableSyncPlayGroupUpdateMessage) Get() *SyncPlayGroupUpdateMessage {
	return v.value
}

func (v *NullableSyncPlayGroupUpdateMessage) Set(val *SyncPlayGroupUpdateMessage) {
	v.value = val
	v.isSet = true
}

func (v NullableSyncPlayGroupUpdateMessage) IsSet() bool {
	return v.isSet
}

func (v *NullableSyncPlayGroupUpdateMessage) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSyncPlayGroupUpdateMessage(val *SyncPlayGroupUpdateMessage) *NullableSyncPlayGroupUpdateMessage {
	return &NullableSyncPlayGroupUpdateMessage{value: val, isSet: true}
}

func (v NullableSyncPlayGroupUpdateMessage) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSyncPlayGroupUpdateMessage) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


