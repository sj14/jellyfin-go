...
type ConfigurationAPI interface {
	...
	*/
	...
	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return ApiUpdateMediaEncoderPathRequest

	Deprecated
	*/
	UpdateMediaEncoderPath(ctx context.Context) ApiUpdateMediaEncoderPathRequest

	// UpdateMediaEncoderPathExecute executes the request
	// Deprecated
	UpdateMediaEncoderPathExecute(r ApiUpdateMediaEncoderPathRequest) (*http.Response, error)
...
type ApiUpdateMediaEncoderPathRequest struct {
	ctx context.Context
	ApiService ConfigurationAPI
...
func (r ApiUpdateMediaEncoderPathRequest) MediaEncoderPathDto(mediaEncoderPathDto MediaEncoderPathDto) ApiUpdateMediaEncoderPathRequest {
	r.mediaEncoderPathDto = &mediaEncoderPathDto
	return r
}

func (r ApiUpdateMediaEncoderPathRequest) Execute() (*http.Response, error) {
	return r.ApiService.UpdateMediaEncoderPathExecute(r)
}
...
Deprecated
*/
func (a *ConfigurationAPIService) UpdateMediaEncoderPath(ctx context.Context) ApiUpdateMediaEncoderPathRequest {
	return ApiUpdateMediaEncoderPathRequest{
		ApiService: a,
		ctx: ctx,
	}
...
// Execute executes the request
// Deprecated
func (a *ConfigurationAPIService) UpdateMediaEncoderPathExecute(r ApiUpdateMediaEncoderPathRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ConfigurationAPIService.UpdateMediaEncoderPath")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/System/MediaEncoder/Path"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.mediaEncoderPathDto == nil {
		return nil, reportError("mediaEncoderPathDto is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.mediaEncoderPathDto
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["CustomAuthentication"]; ok {
